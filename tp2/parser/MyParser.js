...
class MyParser {
	...
	*/
	...
	parseDirectionalLight(prim) {
		...
		// check for errors
		if (![prim.color, prim.position].every((value) => value !== undefined)) {
			console.error('Error in MyParser.parseDirectionalLight : missing attributes');
			return
		}
	...
	getDistanceNurb(prim){
		...
		const firstRowfirstPoint = new THREE.Vector3(prim.controlpoints[0].x, prim.controlpoints[0].y, prim.controlpoints[0].z);
		const firstRowlastPoint = new THREE.Vector3(prim.controlpoints[lenghtRow - 1].x, prim.controlpoints[lenghtRow - 1].y, prim.controlpoints[lenghtRow - 1].z);
		const LastRowLastPoint = new THREE.Vector3(prim.controlpoints[nPoints - 1].x, prim.controlpoints[nPoints - 1].y, prim.controlpoints[nPoints - 1].z);
	...
	parseLod(data, name, parent_material, parent_castshadows, parent_receiveshadows){
		...
		if(lod.type !== 'lod') {
			console.error(`Error in MyParser.parseLod at lod "${name}": invalid or undefined type`);
			return
		}
		const lodNodes = lod.lodNodes
		if(lodNodes.length <= 1) {
			console.error(`Error in MyParser.parseLod at lod "${name}": declare less that one descendents (representations)`);
		}
		...
		for(let i = 0; i < lodNodes.length; i ++){
			...
			if (mindist === undefined || typeof mindist !== 'number' || distSet.has(mindist)) {
				console.error(`Error in MyParser.parseLod at lod "${name}": invalid, undefined or repeted mindist values`);
				return
			}
